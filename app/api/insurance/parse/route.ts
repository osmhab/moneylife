...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    - occurrence ("mois", "trimestre" ou "annee")
    - valeurRachatActuelle (number)
    - projectionAssureur (number)
    - capitalDecesFixe (number)
    - typeCapitalDeces ("fixe" si le contrat garantit un capital décès en CHF, "primes" si la prestation décès est la restitution des primes versées ; "" si le document ne le dit pas)
