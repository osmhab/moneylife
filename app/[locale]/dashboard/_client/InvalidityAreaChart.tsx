  ...
  return years.map((y, idx) => {
    ...
    if (yearIndex < 2) {
      ...
      ai =
        ...
        0;
      // On utilise nos fonctions "blindées" qui priorisent le scan Vita
      const lppInvaliditeReal = calcRenteInvaliditeLPP(client, 'maladie');
      const lppEnfantReal = calcRenteEnfantInvaliditeLPP(client, 'maladie');
      
      lpp = lppInvaliditeReal + lppEnfantReal;
      laa = 0;
    }
...
function buildInvaliditeTimelineAccident(
  ...
  return years.map((y, idx) => {
    ...
    if (yearIndex < 2) {
      ...
      // On injecte la valeur réelle LPP Accident du certificat
      // La coordination (cap) est déjà gérée dans computeInvaliditeAccident, 
      // mais on s'assure de prendre la base réelle du scan.
      const lppBaseAccident = calcRenteInvaliditeLPP(client, 'accident');
      
      // Si le scan dit 0 (ex: Groupe Mutuel), alors lpp sera 0.
      lpp = Math.min(lppBaseAccident, res.phaseRente.annual.lppAfterCap);
    }
...
export default function InvalidityAreaChart({ targetUid }: { targetUid?: string }) {
  ...
  // 1) UID cible : soit targetUid (admin), soit auth.uid (client)
  React.useEffect(() => {
    if (targetUid) {
      setUid(targetUid);
      setLoading(true);
      ...
      <CardHeader className="flex flex-row items-center justify-between gap-3 px-0 pb-4">
        <div>
          <CardTitle className="text-base sm:text-lg">
            Cas Invalidité — projection jusqu&apos;à 65 ans
          </CardTitle>
          {firstPoint && (
...
/>
          ...
          Les chiffres affichés ici sont issus de calculs de lois (AVS) combinés aux données de votre certificat de caisse pension et aux indemnités journalières que vous avez renseignées lors du questionnaire. 
          La lacune correspond à votre revenu actuel, moins les prestations AVS et LPP
        </p>
      </CardContent>
    </Card>
  );
