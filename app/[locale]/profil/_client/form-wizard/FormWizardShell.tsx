...
export default function FormWizardShell<TForm extends MinimalForm>({
  ...
  const { getValues, trigger, setFocus, watch } = form;
const watchedAll = watch(); // force re-render sur toute modif RHF
...
const goPrev = () => {
  ...
  // Réutilisable : save + redirect dashboard
  const submitAndRedirect = async () => {
    // VIBRATION DE SUCCÈS FINAL
    ...
    const submit = form.handleSubmit(
      async (values) => {
        try {
          ...
          alert("Une erreur est survenue lors de l’enregistrement. Merci de réessayer.");
        } finally {
          setIsSaving(false);
        }
      },
  ...
  const ActiveSectionRenderer = () => {
      ...
      return (
        <div className="w-full max-w-screen-sm mx-auto">
          ...
          <div
            ...
            aria-live="polite"
          >
            {/* onNext ici doit bypass la validation (le lien manuel doit avancer) */}
            <ScanLPPSection
              form={form as any}
              onNext={goNext}
            />
    ...
    // 🔹 Cas spécial : Affiliation LPP → permettre de passer toutes les étapes LPP
    if (id === "affilie-lpp") {
      return (
        <div className="w-full max-w-screen-sm mx-auto">
          <header className="pt-2 pb-6">
  ...
  // 🔒 Désactiver "Suivant" sur l'étape scan-lpp tant que le scan n'est pas terminé
  const isScanStep = currentId === "scan-lpp";
const scanDone = watch("Enter_lppScanDone" as any) === true;
const disablePrimaryBecauseScan = isScanStep && !scanDone;
      ...
      <div className="absolute left-0 right-0 bottom-0 z-30 px-4 pb-[env(safe-area-inset-bottom)] pt-2 bg-gradient-to-t from-background via-background/95 to-transparent">
        <div className="max-w-screen-sm mx-auto w-full space-y-1 mb-8">
          <button
            type="button"
            onClick={onPrimary}
            disabled={(!isLast && !isCurrentValid) || isGlobalLoading || isSaving || disablePrimaryBecauseScan}
            className={`w-full h-12 rounded-xl text-primary-foreground text-base font-medium transition
            ${
              (!isLast && !isCurrentValid) || isGlobalLoading || isSaving || disablePrimaryBecauseScan
                ? "bg-gray-400"
          ...
          {disablePrimaryBecauseScan ? (
            <p className="text-xs text-muted-foreground text-center">
              Le bouton <span className="font-semibold">Suivant</span> s’activera une fois le scan terminé.
            </p>
          ) : (
