...
export default function AffilieLPPSection({
  ...
  const forceAffilie = statut === 0 && salaire >= LEGAL_SEUIL_ENTREE_LPP;
  ...
  return (
    <div className="space-y-4">
      ...
      {/* ✅ Skip LPP = save + dashboard */}
      <div className="pt-2">
        <Button
          type="button"
          variant="outline"
          className="w-full rounded-xl"
          disabled={!onSkipLpp || isSkipping}
          onClick={async () => {
            if (!onSkipLpp) return;
            try {
              setIsSkipping(true);
        ...
        >
          {isSkipping ? "Enregistrement…" : "Passer les étapes LPP"}
        </Button>

        <p className="text-xs text-muted-foreground text-center mt-2">
          Vous pourrez compléter votre certificat LPP plus tard depuis votre profil.
        </p>
      </div>
    </div>
  );
