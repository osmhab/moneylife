//app/profil/_client/ProfilUnifiedForm.tsx
...
const UnifiedSchema = z.object({
  ...
  // ====== Identité de la Caisse =====
  Enter_lppCaisseNom: z.string().optional(),
  Enter_lppCaisseAdresse: z.string().optional(),
  Enter_lppCaisseTelephone: z.string().optional(),
  Enter_lppCaisseEmail: z.string().optional(),
  Enter_lppCaisseSiteWeb: z.string().optional(),
  Enter_lppFilePath: z.string().optional(),
  Enter_lppOriginalFilename: z.string().optional(),
...
const unsub = subscribeDonneesPersonnelles(u.uid, (data: Partial<ClientData> | null) => {
  ...
  /* ---------- OCR mapping ---------- */
  function aiToFormPatch(ai: any): Partial<UnifiedForm> {
    const patch: Partial<UnifiedForm> = {};
    
    // --- Coordonnées de la Caisse ---
    if (ai.caisseNom) patch.Enter_lppCaisseNom = ai.caisseNom;
    if (ai.caisseAdresse) patch.Enter_lppCaisseAdresse = ai.caisseAdresse;
    if (ai.caisseTelephone) patch.Enter_lppCaisseTelephone = ai.caisseTelephone;
    if (ai.caisseEmail) patch.Enter_lppCaisseEmail = ai.caisseEmail;
    if (ai.caisseSiteWeb) patch.Enter_lppCaisseSiteWeb = ai.caisseSiteWeb;
    ...
    if (typeof ai.capitalDeces === 'number' && ai.capitalDeces > 0) {
      ...
      patch.DecesCapitaux = [...existing, { amount: ai.capitalDeces, plusRente: plusRenteValue, condition: 'np' }];
    }
    return patch;
  }
  ...
  async function handleScanFiles(files: FileList | null) {
    ...
    try {
      ...
      const storagePath = `clients/${u.uid}/lpp_raw/${crypto.randomUUID()}.${ext}`;
      ...
      // Écoute du job en temps réel pour synchroniser le formulaire
      const jobRef = doc(db, "clients", u.uid, "lpp_jobs", jobId);
      const unsub = onSnapshot(jobRef, (snap) => {
        ...
        if (job.status === "DONE_FAST") {
          unsub();
          const patch = aiToFormPatch(job.parsedFast);
          patch.Enter_lppFilePath = storagePath;
          patch.Enter_lppOriginalFilename = f.name;
